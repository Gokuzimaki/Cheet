...
function getNextId($tablename){
  ...
  $run=mysqli_query($host_connli,$query);
...
function produceStates($countryid=0,$stateid=0){
  global $host_tpathplain;
  include($host_tpathplain.'globalsmodule.php');
  ...
  while ($row=mysqli_fetch_assoc($run)) {
    ...
    $statetotal2.='<option value="'.$row['state'].'">'.$row['state'].'</option>';
    $state=$row['state'];
  }
  ...
  $row['selectionoptions']='
    '.$statetotal.'
  ';
  $row['selectionstatestwo']=$statetotal2;
  return $row;
}
...
function briefquery($query,$cline="",$type="",$fforbid=false,$data=array()){
  ...
  if($noroll=="run"){
    // use mysqli_query if no type is specified
    if($type==""&&function_exists('mysql_query')){
      
      $run=mysql_query($query)or die(mysql_error()."Line ".__LINE__." 
        on Line $cline in the calling file. The query is<br>$query");
      $numrows=mysql_num_rows($run);
      ...
      if($multiline==true){
        ...
        $run=mysqli_query($host_connli,$query)or die(mysqli_error($host_connli)." Line ".__LINE__." on Line $cline in the calling file. The query is<br>$query");
...
function getSingleLGA($id){
  ...
  $run=mysqli_query($host_connli,$query)or die(mysqli_error($host_connli)." Real number:".__LINE__." old number  1579");
  $row=mysqli_fetch_assoc($run);
  return $row;
}
          ...
          <tr>
            <td class="center" align="center" valign="top">
              <center>
                <tr>
                 <td class="heading">
                  <img src="'.$logoout.'"  alt="'.$logoalt.'" width="220" height="" style="display: inline-block;" /><br>
                 </td>
                </tr>
                <tr>
                 <td class="heading2">
                ...
                <tr>
                 <td class="minifoot">
                  <div id="sociallinks">
                    <div id="socialholder" name="socialholdfacebook">
                    <a href="##" target="_blank">
                    <img src="'.$host_addr.'images/Facebook-Icon.png" alt="Facebook"/></a></div>
                    <div id="socialholder" name="socialholdlinkedin">
                    <a href="##" target="_blank">
                    <img src="'.$host_addr.'images/Linkedin-Icon.png" alt="LinkedIn"/></a>
                    </div>
                    <div id="socialholder" name="socialholdtwitter">
                    <a href="##" target="_blank">
                    <img src="'.$host_addr.'images/Twitter-Icon.png" alt="Twitter"/></a></div>
                    <div id="socialholder" name="socialholdgoogleplus">
                    <a href="##" target="_blank">
                    <img src="'.$host_addr.'images/google-plus-icon.png" alt="Google+"/></a></div>
                  </div>
                  '.$footer.'
                 </td>
                </tr>
...
function toPeriod($date,$data=array()){
  ...
  $date2 = !isset($data['datetwo'])||(isset($data['datetwo'])&&$data['datetwo']=="")?
  new DateTime(date("Y-m-d H:i:s")):new DateTime($data['datetwo']);
  $dt=" ago";
  if($date1>$date2){
    $dt=" From now";
    $date=$date2;
