<?php 
	...
	function getSingleUserPlain($id,$data=array()){
		global $host_tpathplain;
    	include($host_tpathplain."globalsmodule.php");
		...
		if($numrows>0){
			...
			if($row['usertype']=='user'||(isset($row['usertype'])&&$row['usertype']=='user')){
				// get profile picture
	...
	function getSingleUser($id,$data=array()){
		global $host_addr;
		global $host_tpathplain;
		include($host_tpathplain.'globalsmodule.php');
		$row=array();
		...
		$run2=mysqli_query($host_connli,$query2)or die(mysqli_error($host_connli)." Line 899");
		...
		if($numrows>0){
			...
			// echo $lgquery;
			$lgrun=mysqli_query($host_connli,$lgquery)or die(mysqli_error($host_connli)." Line 50");
			$lgnumrows=mysqli_num_rows($lgrun);
			if($lgnumrows>0){
				while($lgrow=mysqli_fetch_assoc($lgrun)){
	...
	function getAllUsers($viewer,$limit,$user='',$rodata="adminoutputtwo",$type="",$data=array()){
		...
		$queryorder="WHERE (usertype='$user' OR usersubtype='$user')";
		...
		$toptwo='<table id="resultcontenttable" cellspacing="0">
					<thead><tr><th>FullName</th><th>Gender</th><th>Age</th>
		...
		if($numrows>0){
			while($row=mysqli_fetch_assoc($run)){
				$data['row']=$row;
				$outvar=getSingleUser($row['id'],$data);
				// for plain user acc
				$outvartwo=getSingleUserPlain($row['id'],$data);
