<?php
	...
	// this is the default script for parsing multiple portfolio post display related 
	// content on the platform
	// later work would include making a shortcode parsing system available to handle
	// both system and userdefined shortcode.
	// the file will be called/included in whatever portfolio page relies on the theme
	...
	// the index $outs['current_page'] is used for carrying the number
	...
	// the whole portfolio data rendering process 
	...
	if($hijack=="yes"){
		...
		$qextra="WHERE status='active'";

		// check if the user ran a search
		if(isset($_GET['q'])&&$_GET['q']!==""){
			$q=mysql_real_escape_string($_GET['q']);
			$qcat=$qextra==""?"":" AND";
			$qextra.=" $qcat (projecttitle LIKE '%$q%' OR details LIKE '%$q%'
				OR shorttext LIKE '%$q%' OR type LIKE '%$q%' OR tags LIKE '%$q%')";
		}
		...
		$query="SELECT * FROM portfolio $qextra";
		// echo "$query";
		...
		// specifies the type or design output for the sub pagination content
		// that is content that shows up under the pagination section either at
		// the top or bottom
		$b_options['subpaginationtype']=!isset($b_options['subpaginationtype'])?"default":
		$b_options['subpaginationtype'];
		...
		// control whether access to the top/bottom sub pagination data content is
		// allowed 
		$b_options['showtopsubpages']=!isset($b_options['showtopsubpages'])?false:
		$b_options['showtopsubpages'];

		$b_options['showbottomsubpages']=!isset($b_options['showbottomsubpages'])?false:
		$b_options['showbottomsubpages'];
		...
		$portdata=getAllPortfolioEntries('viewer',$limit,"",$b_data);
		unset($b_data);
		// unset($b_options);
		// var_dump($blogdata);
	...
	if(file_exists($host_renderpath."/modules/portfoliopagemultipleparser.php")){
		include($host_renderpath."/modules/portfoliopagemultipleparser.php");
	}else{
		echo '<div class="col-md-12">
			<h1>Error!!!</h1>
			<p>No portfolio page multiple parser module detected in '.$host_rendername.'snippets folder</p>
		</div>';
	}
