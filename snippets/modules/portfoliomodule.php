<?php
	...
	*/
	function getSinglePortfolioEntry($id,$type="",$data=array()){
		...
		$formtruetype="";// carries the name of the form containing edittable content
		$processroute=""; 
		$editroute=""; 
		$totalscripts="";
		...
		// process the $data array contents
		if(isset($data['single'])){
			if(isset($data['single']['datamap'])&&$data['single']['datamap']!==""){
				$datamap=$data['single']['datamap'];
				$gd_testdata=JSONtoPHP($datamap);
			...
			// get the viewer type, that is the class of viewer either 'admin'
			// or 'viewer' to control the output generated
			if(isset($data['single']['viewer'])&&$data['single']['viewer']!==""){
				$viewer=$data['single']['viewer'];
			}

			// strictly for the getsingleportfolioentry function
			// any other indexes passed into the function can be handled here
		...
		if($numrows>0){
			...
			for($i=0;$i<$numrows;$i++){
				...
				$seometakeywords=$outs['seometakeywords']!==""?
				$outs['seometakeywords']:($tags!==""?$tags:$projecttitle);
				
				$status=$outs['status'];
				$hd=$shorttext!==""?$shorttext:
					($details!==""?$details:$projecttitle);
				...
				$mediaquery="SELECT * FROM media WHERE ownerid=$id AND 
								ownertype='portfolio' AND maintype='coverimage' 
								AND status='active'";
				...
				if($mrows>0){
					...
					$row['coverimagedata'][0]=array("location"=>"",
												"medsize"=>"",
												"thumbnail"=>"",
												"caption"=>"",
												"details"=>"",
				...
				$mediaquery="SELECT * FROM media WHERE ownerid=$id AND 
								ownertype='portfolio' AND maintype='bannerimage' 
								AND status='active'";
				$mqdata=briefquery($mediaquery,__LINE__,"mysqli");
				$mrows=$mqdata['numrows'];
				$bannertotal=$mrows;
				...
				// CREATE profpic index for easy managing of coverimage display
				$row['profpicdata']=$row['coverimagedata'][0];
					...
					if($mrows>0){
						for($mq=0;$mq<$mrows;$mq++){
							...
							if(str_replace(" ", "",$jsondata)!==""){
								...
								if($viewer=="viewer"){
									$portvdata['caption']=$caption;

									$jsondata=json_encode($portvdata);
				...
				if($audattach=="yes"){ 
					...
					if($mrows>0){
						for($mq=0;$mq<$mrows;$mq++){
							...
							$audiomapout=array("location"=>"$location",
											"portacaption"=>"$caption",
											"portaembed"=>"$details",
											"disptype"=>"$mtype"
										);
							$originalmap=json_encode($audiomapout);
				...
				if($viewer=="viewer"){
					...
					// get the next and previous portfolio entries
					$nbq="SELECT * FROM portfolio WHERE id>$id 
					AND status='active' LIMIT 0,1";
					$nbrun=briefquery($nbq,__LINE__,"mysqli");
					$nextportdata=isset($nbrun['resultdata'][0])?
					$nbrun['resultdata'][0]:"";
					$nextportdata['numrows']=$nbrun['numrows'];

					if(isset($nbrun['resultdata'][0])&&
						$nbrun['resultdata'][0]['id']>0){

						$row['nextportfoliodata']=$nbrun['resultdata'][0];
							

					}
					$row['nextportfoliodata']['numrows']=$nbrun['numrows'];
					unset($nbrun);


					$pbq="SELECT * FROM portfolio WHERE id<$id 
					AND status='active' LIMIT 0,1";
					$pbrun=briefquery($pbq,__LINE__,"mysqli");
					if(isset($pbrun['resultdata'][0])&&
						$pbrun['resultdata'][0]['id']>0){

						$row['prevportfoliodata']=$pbrun['resultdata'][0];
					}
					$row['prevportfoliodata']['numrows']=$pbrun['numrows'];
					unset($pbrun);
	...
	*/
	function getAllPortfolioEntries($viewer,$limit,$type="",$data=array()){
		...
		if(isset($data['single']['type'])&&$data['single']['type']!==""){
			$singletype=$data['single']['type'];
			$singletype['single']['viewer']=$viewer;
		}
