<?php
	...
	function getSingleComment($commentid,$type="",$data=array()){
		global $host_addr,$host_target_addr,$host_admin_title_name;
		$row=array();
	...
	function getAllComments($viewer,$limit,$blogpostid,$data=array()){
	    ...
	    $paginateout="";
	    $extraqdata="";
	    $alevel=isset($_SESSION['accesslevel'.$logpart.''])?
	    $_SESSION['accesslevel'.$logpart.'']:0;
	    if(is_numeric($blogpostid)){
		    if($viewer=="admin"){
			    if($limit=="" && $blogpostid==""){
				    $query="SELECT * FROM comments WHERE status!='disabled' 
				    		order by id,status desc LIMIT 0,15";
				    $rowmonitor['chiefquery']="SELECT * FROM comments WHERE status!='disabled' 
	...
	function getSingleBlogEntry($blogentryid,$type="",$data=array()){
	  include('globalsmodule.php');
	  $row=array();
	  ...
	  AND maintype='coverphoto' AND status='active' ORDER BY id DESC";
	  ...
	  //control variables for varying editable type content, 
	  // for video audio and schedule information
	  $schedulescriptout='';
	  $scheduleedit="";
	  $selectaudioscripts='';
	  $editaudiocontentout="";
	  ...
	  if ($blogentrytype==""||$blogentrytype=="normal") {
		    ...
		    <div id="formend">
		      ...
		      <input type="file" placeholder="Choose image" name="bannerpic" class="form-control"/>
		    </div>
		    ...
		    $mediaquery="SELECT * FROM media WHERE ownerid=$id AND ownertype='blogentry' 
		    			AND maintype='gallerypic' AND status='active' ORDER BY id DESC";
		    $mediarun=mysql_query($mediaquery)or die(mysql_error()." Line ".__LINE__);
		    $medianumrows=mysql_num_rows($mediarun);
		    $thumbstack="";
		    $locationstack="";
		    ...
		    if($betype=="local"){
		      ...
		      ownertype='blogentry' AND maintype='blogentryvideo' AND status='active' ORDER BY id DESC";
		    ...
		    $editvideocontentout='
		    	<div class="col-md-12 emu-row" data-name="">
			        ...
			        <div class="col-md-12 emu-row" data-name="embedvideo" >
			            Video Embed Code *<br>
			            <textarea placeholder="Place your embed code here" name="videoembed" 
			            	class="form-control"
			            >'.$becode.'</textarea>
			            '.$videoouttwo.'
			        </div>
	  ...
	  $row['adminoutput']='
		    <tr data-id="'.$id.'">
		    	<td class="tdimg"><img src="'.$host_addr.''.$coverphoto.'"/></td>
		    	<td>'.$link.'</td>
		    	<td>'.$blogtypedata['name'].'</td>
		    	<td>'.$blogcategorydata['catname'].'</td>
		    	<td>'.$commentdata['commentcount'].'</td>
		    	<td>'.$views.'</td>
		    	<td>'.$entrydate.'</td>
		    	<td>'.$modifydate.'</td>
		    	<td>'.$status.'</td>
		    	<td name="trcontrolpoint">
		    		<a href="#&id='.$id.'" name="edit" data-type="editsingleblogpost" 
		    		data-divid="'.$id.'">Edit</a>
		    	</td>
		    </tr>
		    <tr name="tableeditcontainer" data-state="inactive" data-divid="'.$id.'">
		    	<td colspan="100%">
	  ...
	  $row['adminoutputthree']='
		    <tr data-id="'.$id.'">
		      <td class="tdimg"><img src="'.$host_addr.''.$coverphoto.'"/></td>
		      <td>'.$title.'</td><td>'.$blogtypedata['name'].'</td>
		      <td>'.$blogcategorydata['catname'].'</td><td>'.$postperiod.'</td>
		      <td>'.$scheduledpost.'</td><td>'.$entrydate.'</td><td>'.$status.'</td>
		      <td name="trcontrolpoint">
		      	<a href="#&id='.$id.'" name="edit" 
		      		data-type="editsingleblogpost" 
		      		data-divid="'.$id.'">Edit</a>
		      </td>
		    </tr>
		    <tr name="tableeditcontainer" data-state="inactive" data-divid="'.$id.'">
