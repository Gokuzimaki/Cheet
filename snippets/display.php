...
if($test_gdunit===true||$test_gdunit>-1){
...
}else if($displaytype=="createbooking"){
	// echo $displaytype;
	...
	echo $displaytype;
}elseif($displaytype=="users"){
	// echo $displaytype;
	include('createuseracc.php');
}elseif($displaytype=="serviceproviders"){
	// echo $displaytype;
	include('createclientacc.php');
}elseif($displaytype=="incidents"){
	// echo $displaytype;
	...
	$displaytype=="portfoliowelcomemsg"||
	...
	if($datamap!==""){
		...
		$gdtest_data=JSONtoPHP($datamap);
		$gd_testoutput=$gdtest_data['arrayoutput'];
		// viewtype for form file section display
		$_gdunit_viewtype=$gd_testoutput['vt']?$gd_testoutput['vt']:"create";

		// viewertype for user class type, data function access.
		$_gdunit_viewertype=$gd_testoutput['vtype']?$gd_testoutput['vtype']:"admin";
 ...
 if($emaildata['testresult']=="unmatched"){
	...
	$displaytype=="usersnew"||
	$displaytype=="usersedit"){
	// echo $displaytype;
	include('createuseracc.php');
}elseif($displaytype=="serviceproviders"||
	$displaytype=="serviceprovidersnew"||
	$displaytype=="serviceprovidersedit"||
	...
	# code...
	$outs=getAllBlogTypes($extraval,"all");
	echo $outs['adminoutput'];
	...
	echo $select;
}elseif ($displaytype=="getqgroupcourses") {
	# code...
	$tid=mysql_real_escape_string($_GET['tid']);

	$select='<option value="">--Choose A Question Group First--</option>';

	// get the question group data 
	$outs=getSingleQuestionGroup($tid);
	
	// check to see if the question group has courses attached
	if($outs['qscdata']['total']>0){
		for ($i=1; $i <=$outs['qscdata']['total']; $i++) { 
			$n=$i-1;
			$cdata=$outs['qscdata'][$n];
...
echo $outs['adminoutput'];
	...
	// echo $query2;
	// echo $type;
	$run=mysqli_query($host_connli,$query2)or die(mysqli_error($host_connli));
	$numrows=mysqli_num_rows($run);
	$otype=$type;
	...
	if($numrows>0){
		if($type==""){
			...
			// $rdata['user']=$usertype;
			// $outs=getAllUsers($viewer,$limit,"",$rdata);
		...
		if($typeout==""){
			...
			echo json_encode(array("success"=>"true","msg"=>"No database Entries",
				"resultcount"=>"0"));
		}
	}
	unset($data);
	...
	$run=mysqli_query($host_connli,$query)or die(mysqli_error($host_connli)." Line 287");
	$numrows=mysqli_num_rows($run);
	if($numrows>0){
		$adminoutput="";
		while($row=mysqli_fetch_assoc($run)){
