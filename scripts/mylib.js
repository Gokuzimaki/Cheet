...
if (host_addr.indexOf('ngrok.io/') > -1) {
    host_addr = "http://" + location.hostname + "/cheetmini/";
}
//variable for holding filemangerheader title
var host_admin_title_name = "Cheet";
...
// fieldValueVerification - $feverify
/*
...
*   it is basically a jquery object - $(this)
*
*   @param array negateset carries the set of values that should be removed
*
*   @return null
*/
function negateDefault(el,negateset=[]){

    // console.log(el," value:"+el.val());

    // this variable determines if the date removal from the 
    // el value should be done or not in case there are no
    var donegate=true;
    
    // get the current element value
    var curval=el.val();
    
    // get the current negation data set for the element
    // in the event it carries the required attribute
    var curnegate=el.attr("data-negateset");
    if(curnegate===null||curnegate===undefined||curnegate===NaN){ 
        curnegate="";
        donegate=false;
    }

    if(curnegate!==""){
        curnegate=JSON.parse(curnegate);
    }  

    // check the negateset for necessary values 
    if(donegate==false){
        if(negateset.length>0){
            curnegate=negateset;
            donegate=true;
        }
    }
    ...
    if(donegate==true){
        for(var i=0;i<curnegate.length;i++){
            var cn=curnegate[i];
            if(cn==curval){
                // reset the element value to nothing
                el.get(0).value="";
                // console.log(el," value:"+el.val());
...
// end tel field values correction
...
/** function isPrev - (image Selection preview)
    *   Parameters (el)
    *
    *   This function allows the presentation of chosen image from the file element 
    *   via the lightbox image preview library. The function is to be bound to the 
    *   onChange event for the file element for optimal performance, it also requires
    *   bootstrap based classes to function properly.
    *   IMPORTANT!!! this function best in browsers that support the reader object for 
    *   file data.
    *
...
function isPrev(el,nextel=""){
    ...
    if(typeof el=="string"){
        ...
        // this assumes that el is a jquery selector string hence we convert it into a 
        // jquery object
        el=$(el);
    }   
    ...
    // default name for current lightbox gallery 
    var tgal = Math.floor(Date.now() / 1000);
    ...
    if (el[0].files[0]) {
        ...
        // create a new reader object for getting the 
        var reader = new FileReader();

        reader.onload = function(e) {

            var ftype=getExtension(el.val());
            if(ftype['type']=="image"){
                // get the image data element
                result = e.target.result;
                // create the image light box display
                var lb=`<a href="`+result+`" data-src="`+result+`"
...
/** function multipleElGenerator
    ...
    *                data=[])
    *   
    *   Creates form fields in multiple element groups in repeating format
    ...
    *   element container, the value is either more than or less than 'cucountel' and 
    *   depending on the value an increment or decrement in the number of entries will occur. 
    *
    ...
    *
    *   @param array 'data' is an array that carries any other information for use in the 
    *   function
    *
    *   @return null
    *
    ...
    *
*/
function multipleElGenerator(element, entryel="", groupparent="", curcountel=0, curnextcount=0, countel="",data=[]) {
    ...
    if(curnextcount>0){
        if (podtype == "" || podtype == "increment") {
            ...
            if (podtype == "") {
                ...
                console.log("Type of: "+typeof elgroup,elgroup);
                ...
                // these arrays carry the config for new toolbar components in tinymce 
                // initialization
                var mceoptions = [];
                var domce = "";
                var cmcecount = 0;
                ...
                if (mceelements.length > 0) {
                    ...
                    // run a for loop on the mce elements to process them for the 
                    // new content
            ...
            for (var i = gc.length - 1; i > curnextcount - 1; i--) {
                ...
                if (typeof ($('' + groupparent + ' .multi_content_hold')[i]) !== "undefined") {
                    // block deletion of the progenitor element
                    if($('' + groupparent + ' .multi_content_hold')[i].getAttribute("data-type")!=="triggerprogenitor"){
...
// for general form addition via link with 'triggerformaddlib' data-type attribute
$(document).on("click", "a[data-type=triggerformaddlib]", function() {
    ...
    // destruction block for certain frequently used js libraries
    // select
    if($.fn.select2){
    ...
    // check for tinymce elements in the elgroup and proceed to augment them as 
    // appropriate
    var mceelements = elgroup.find('[data-type=tinymcefield]');
    ...
    // remove any progenitor details from the cloned element
    var cid = $(this).parent()
    .find('div[data-type=triggerprogenitor]').attr("data-cid");
    elgroup.removeAttr("data-cid");
    elgroup.attr("data-type", "triggerprogeny");
    var hlabelcount=elgroup.find('.multi_content_countlabels').length;
    // console.log(hlabeltext);
    // console.log("Real group - ", elgroup," ",elgroup.find('.multi_content_countlabels'));
    if(hlabelcount>0){
...
function filter(selector, query,type='text',attr="",phide="") {
    ...
    // phide is the selector for a seperate element to be shown or hidden
    ...
    if(phide==""){
        ...
        if(type=="text"){
            $(selector).each(function() {
                ($(this).text().search(new RegExp(query, "i")) < 0) ? $(phide).addClass('hidden') : $(this).removeClass('hidden');
            });
        }else if(type=="html"){
            $(selector).each(function() {
                ($(this).html().search(new RegExp(query, "i")) < 0) ? $(phide).addClass('hidden') : $(this).removeClass('hidden');
            });
        }else if(type=="attribute"){
            $(selector).each(function() {
                ($(this).attr(attr).search(new RegExp(query, "i")) < 0) ? $(phide).addClass('hidden') : $(this).removeClass('hidden');
            });
        }
    }
}
            ...
            $(this).keyup(function(event) {
                ...
                if(isNumber(maxcount)){
                    ...
                    if(checkElem.is('.wMax-view')){
                        ...
                        // create the wMax elem object and place it after the current 
                        // element
                        var tval = $(this).val();
...
/*rangedSlider*/
// this jquery plugin handles the creation and function of the ranged slider element
// in the 'adminthemerlte' theme.
(function($){
    // rangeSlider Setup
    $.fn.rangedSlider=function(selector=""){
        // console.log(this);
        selector==""?selector='div[data-rangeslider] .slider, .range-slider .slider':selector=selector;
        if($(this).get(0).hasAttribute('rangedSlider-selector')){
            selector=$(this).attr('rangedSlider-selector')!==""?$(this).hasAttribute('rangedSlider-selector'):selector;
        ...
        $(selector).each(function () {
            ...
            $this.slider({
                ...
                /*if(data.length==2){
                    var firstel=el.parent().find('.first-value span');
                    var lastel=el.parent().find('.last-value span');
                    firstel.css("display","block").text(data[0]);
                    lastel.css("display","block").text(data[1]);
                }*/
