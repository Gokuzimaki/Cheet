/* contactus form
================================================== */
// control variable for handling main form submission
mainformit="";
mainformittwo="";
monitor="off";
elem="";
...
// imporove/overide jqueryval method
$.fn.pVal = function(){
    var $this = $(this),
        val = $this.eq(0).val();
...
function handleContactFormTwo() {
		...
		// console.log("hello, right turn");
		event.preventDefault();
		var form=$("form#contact-form");
		var name=$('form#contact-form input[name=name]');
		var email=$('form#contact-form input[name=email]');
		var subject=$('form#contact-form input[name=subject]');
		var message=$('form#contact-form textarea[name=message]');
		console.log(name.val(),name.attr('placeholder')," ",email.val()," ",subject.val()," ",message.val());
		if(name.val()==""){
			doFJCALert(name, "", "Please provide your fullname.", "");
			monitor="on";
		}else if (email.val()=="") {
			doFJCALert(email, "", "Provide your email.", "");
			/*var monitor=emailValidatorReturnable(email);
			monitor="on";*/
		}else if (subject.val()=="") {
			doFJCALert(subject, "", "State the subject.", "");
			monitor="on";
		}else if(message.val()==""){
			doFJCALert(message, "", "Provide your message.", "");
			monitor="on";
		}
		if(monitor=="off" && email.val()!==""){
			var outit=emailValidatorReturnableTwo(email.val());
			
			if(outit['status']=="false"){ 
				console.log("werey re o");
				doFJCALert(email, "",outit['errormsg'] , "");
				monitor="on";
			}
		}else if(monitor=="off"){
			var result;
			$.ajax({
				...
				success: function(msg) {
					if (msg.replace(/\s\s*/g,"") == 'OK') {
						result = '<div class="alert alert-success"><i class="fa fa-times-circle close"></i> Your message has been sent successfully, we will get back to you shortly, thank you.</div>';
						// $('form#contact-form').reset();
					} else {
						result = '<div class="alert alert-error"><i class="fa fa-times-circle close"></i>' + msg + '</div>';
					}
					$("#contact-form #formstatus").html(result);

				},
				error: function() {

					result = '<div class="alert alert-error"><i class="fa fa-times-circle close"></i>There was an error sending the message!</div>';
...
function s_vegas ()
{
	...
	var slides_2 = [{
		name: "Team Coaching Activation",
		// src: 'images/fcimages/a1.jpg'
		src: 'images/bigstock-African-businessman-shaking-ha-38567197.jpg'
	}, {
		name: "International Marketing Partnerships",
		src: 'images/commercial.jpg'
	}, {
		name: "Brand Management Consultancy",
		src: 'images/brandlife.jpg'
	}];
...
$(document).ready(function(){
	...
	$(document).on('change','input,select,textarea',function(){
		$(this).parent().find('.alert-specialv.validation-error').remove();
	});
